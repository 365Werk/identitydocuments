        ...
        foreach($document->raw as $raw){
            if($raw['converted'] != "" && ! Str::contains($MRZ_string,$raw['original'])){
                ...
                if($surname === $raw['converted']) {
                    ...
                    if(!$document->matched->surname || sizeof($document->matched->surname['value'])>sizeof($raw['value']))
                    $document->matched->surname = [
                        'value' => $raw['original'],
                        'confidence' => 0.9,
                    ];
                ...
                if($given_names === $raw['converted']) {
                    ...
                    if(!$document->matched->given_names || sizeof($document->matched->given_names['value'])>sizeof($raw['value']))
                    $document->matched->given_names = [
                        'value' => $raw['original'],
                        'confidence' => 0.9,
                    ];
